...
void raw_events_ana(){
  ...
  */
  ...
  TString outcan = "./output_can/can_raw_eve_run"+ to_string(runstart)+"-"+to_string(runstop)+".root";
  TFile *canfile= new TFile(outcan,"recreate");
  AtTpcMap *map = new AtTpcMap();
  map->ParseXMLMap(mapDir.Data());
  map->GeneratePadPlane();

  Double_t SR = 12.5; // sampling rate: [MHz]
  Double_t SP = 1/SR; // sampling period: 80us (1 Time Bucket)
  ...
  TH1F *histADCvT = new TH1F("histADCvT", "histADCvT", 512, 0, 41);
  ...
  std::array<TH2F *, 12> histADCPerSmallPad = {
    new TH2F("histADCPerSmallPad1", "histADCPerSmallPad1", 512, 0, 512, 10000, -5000, 5000),
  ...
  /*
  std::array<TH2F *, 12> histRawADCPerSmallPad = {
    new TH2F("histRawADCPerSmallPad1", "histRawADCPerSmallPad1", 512, 0, 512, 65536, 0, 65536),
  ...
  */
  std::array<TH2F *, 8> histADCPerBigPad = {
    new TH2F("histADCPerBigPad1", "histADCPerBigPad1", 512, 0, 512, 10000, -5000, 5000),
  ...
  /*
  std::array<TH2F *, 8> histRawADCPerBigPad = {
    new TH2F("histRawADCPerBigPad1", "histRawADCPerBigPad1", 512, 0, 512, 65536, 0, 65536),
  ...
  */
  std::array<TH2F *, 20> histADCPerPad = {
    new TH2F("histADCPerPad1", "histADCPerPad1", 512, 0, 512, 10000, -5000, 5000),
  ...
  /*
  std::array<TH2F *, 20> histRawADCPerPad = {
    new TH2F("histRawADCPerPad1", "histRawADCPerPad1", 512, 0, 512, 65536, 0, 65536),
  ...
  */
  ...
  //read the file and fill data to hist
  //   std::vector runNums = {5120};
  //   for (int runNum: runNums) {
  for (int runNum=runstart; runNum < runstop + 1; runNum++) {
	...
	for (int j = 0; j < 512; j++) {
      ...
      if(i%1000==0){
	cout<<"Getting data: run "<<runNum<<", "<<100*i/nUnpackEvents<<" % finished!           \r"<<flush;
      }
    }
    // Close files.
    unpackFile->Close();
  ...
  histADCvTB->SetDirectory(0);
  histADCvTB->Draw();
  histADCvTB->GetXaxis()->SetTitle("TB");
  histADCvTB->GetYaxis()->SetTitle("#Sigma ADC");
  canfile->WriteTObject(histADCvTB);

  TCanvas *c2 = new TCanvas();
  histADCvT->SetDirectory(0);
  histADCvT->Draw();
  histADCvT->GetXaxis()->SetTitle("T [#mus]");
  histADCvT->GetYaxis()->SetTitle("#Sigma ADC");
  canfile->WriteTObject(histADCvT);
  /*
