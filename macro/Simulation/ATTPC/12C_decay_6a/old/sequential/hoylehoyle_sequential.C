...
int sequential_hoyle(Int_t nEvents=1000, Int_t subnum=0){

  //set situation
  Double_t p_hoyle=0; //z momentum hoyle in MeV
  Double_t k_beam=10; //beam energy in MeV
  ...
  const Double_t Ex_hoyle = 0.00765; // 12C(0+2) excitation energy [GeV]
  ...
  Double_t uniform_ran, p_gx, p_gy, p_gz, p_ga, Esum6, Esum12, psum12, Ex12c, k12c;
  Double_t px[3];
  Double_t py[3];
  Double_t pz[3];
  Double_t E[3];
  ...
  Double_t pz12[3];
  Double_t weight;
  Double_t T_beam=k_beam/1000;        // beam (12C) kinetic energy [GeV]
  Double_t E_beam=c12_mass + T_beam;  // beam (12C) total energy [GeV]
  Double_t Pz_beam=sqrt(E_beam*E_beam - c12_mass*c12_mass);  // beam (12C) Pz [GeV]
  TLorentzVector beam(0.0, 0.0, Pz_beam, E_beam);  // 4-mom. vector for the beam
  TLorentzVector target(0.0, 0.0, 0.0, c12_mass);     // 4-mom. vector for the target
  TLorentzVector W = beam + target;
  //  TLorentzVector c12_hoyle(0.0,0.0,p_hoyle,E_hoyle);
  TLorentzVector *c12_hoyle[2];
  TLorentzVector *be8_tmp[2];
  ...
  TH1F *h_pgx=new TH1F("h_pgx","px of CM",100,-1,1);
  TH1F *h_pgy=new TH1F("h_pgy","py of CM",100,-1,1);
  TH1F *h_pgz=new TH1F("h_pgz","pz of CM",100,-1,1);
  TH1F *h_Esa=new TH1F("h_Esa","sum energy of alpha",100,0,1);
  TH1F *h_Ex=new TH1F("h_Ex","Excitation energy of hoyle",100,7,8);
  TH1F *h_k=new TH1F("h_k","kinetic energy of hoyle",100,-5,5);
  ...
  //  TString outdata="data1/out_sequential_"+ to_string(subnum) +".root";
  ...
  for(int i=0;i<nEvents;i++){
    ...
    //event generation 12c+12c->hoyle+hoyle
    entry++;
    event.SetDecay(W, 2, mass_12c12c); //mass_12c12c=(mass12c+ex_hoyle, mass12c_ex_hoyle
    do {
    ...
    TLorentzVector C12_1 = *event.GetDecay(0);
    TLorentzVector C12_2 = *event.GetDecay(1);

    c12_hoyle[0]= *event.GetDecay(0);
    c12_hoyle[1]= *event.GetDecay(1);

    //hoyle->3alpha 1
    if(event_8BeAlpha[0].SetDecay(c12_hoyle[0], 2, be8alpha_mass)){
      do {
	...
	uniform_ran = gRandom->Uniform(0.,weight_max);
      ...
      be8_tmp[0]=*event_8BeAlpha[0].GetDecay(0);
      p_alpha[0]=event_8BeAlpha[0].GetDecay(1);
      n_decay+=1;
      
      if(event_2alpha[0].SetDecay(be8_tmp[0],2,alphaalpha_mass)){
	do {
	...
	p_alpha[1]=event_2alpha[0].GetDecay(0);
	p_alpha[2]=event_2alpha[0].GetDecay(1);
	n_decay+=2;
      }
    }

    //hoyle->3alpha 2
    if(event_8BeAlpha[1].SetDecay(c12_hoyle[1], 2, be8alpha_mass)){
      do {
	...
	uniform_ran = gRandom->Uniform(0.,weight_max);
      ...
      be8_tmp[1]=*event_8BeAlpha[1].GetDecay(0);
      p_alpha[3]=event_8BeAlpha[1].GetDecay(1);
      n_decay+=1;
      
      if(event_2alpha[1].SetDecay(be8_tmp[1],2,alphaalpha_mass)){
	do {
	...
	p_alpha[4]=event_2alpha[1].GetDecay(0);
	p_alpha[5]=event_2alpha[1].GetDecay(1);
	n_decay+=2;
      }
    }

    for(int k=0;k<2;k++){
      px_hoyle[k]=c12_hoyle[k].Px()*GtoM;
      py_hoyle[k]=c12_hoyle[k].Py()*GtoM;
      pz_hoyle[k]=c12_hoyle[k].Pz()*GtoM;
      E_hoyle[k] =c12_hoyle[k].E()*GtoM;

      px_be8[k]=be8_tmp[k].Px()*GtoM;
      py_be8[k]=be8_tmp[k].Py()*GtoM;
      pz_be8[k]=be8_tmp[k].Pz()*GtoM;
      E_be8[k] =be8_tmp[k].E()*GtoM;
    }
    for(int j=0;j<6;j++){
      ...
      /*
      //chenge GeV to MeV
      px_alpha[j]=px_alpha_tm*GtoM;
      py_alpha[j]=py_alpha_tm*GtoM;
      pz_alpha[j]=pz_alpha_tm*GtoM;
      E_alpha[j] =E_alpha_tm*GtoM;
      px_be8=px_be8_tm*GtoM;
      py_be8=py_be8_tm*GtoM;
      pz_be8=pz_be8_tm*GtoM;
      E_be8 =E_be8_tm*GtoM;
      */
    ...
    psum12=palpha[0]+palpha[1]+palpha[2];
