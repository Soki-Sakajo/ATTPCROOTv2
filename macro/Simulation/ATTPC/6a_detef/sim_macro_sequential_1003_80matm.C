...
int sim_macro_sequential_1003_80matm(Int_t subnum=0){ 
  ...
  TString infname =
    outdir + "attpcsim_sequential_hoylehoyle_80matm_" + to_string(subnum) + ".root";
  ...
  TString digifname =
    ...
    outdir + "output_digi_rcnp_sequential_hoylehoyle_80matm_" + to_string(subnum) + ".root";
  ...
  TString outfname =
    outdir + "sim_macro_sequential_hoylehoyle_80matm_" + to_string(subnum) + ".root";
  ...
  anadata->Branch("startKi",&ka,"ka[6]/D");
  ...
  TH1F *h_kial=new TH1F("h_kial","kinematic energy of alpha",100,-1,15);
    ...
    //    else if (i%2==1 && n==7 && n_d[i-1]==1) {
      ...
      for(int j=0; j<n ; j++){	
        ...
        for(int k=0; k<np ; k++){
	      ...
	      ka[j]=(px[j]*px[j]+py[j]*py[j]+pz[j]*pz[j])/alpha_mass/2;
	      h_kial->Fill(ka[j]);
              scat_start[j]=false;
            }
  ...
  TCanvas *can1=new TCanvas("hist","hist");
  h_kial->Draw();
  //output for canvas
  ...
  TString outcan =
    outdir + "can_sim_macro_sequential_hoyle_80matm_"+ to_string(subnum) +".C";
  can->SaveAs(outcan);
