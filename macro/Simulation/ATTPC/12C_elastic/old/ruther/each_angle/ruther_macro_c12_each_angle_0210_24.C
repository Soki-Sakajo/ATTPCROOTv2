...
int ruther_macro_c12_each_angle_0210_24(){
  //int ruther_macro_c12_each_angle_0210_24(Int_t subnum=0){ 
  ...
  for(int j=0; j<Entry1; j++){
    ...
    if(theta1[j]-theta_min<0){continue;}
    
    if(n_p1==2 && fabs(z1[0]-z1[1])<dz){
      z_n=int(z1[0]/dz);
      a_n=int((theta1[j]-theta_min)/da);
      ev_ve[z_n][a_n].push_back(j);
  ...
  for(int i=0;i < ev_ve.size(); i++){
    ...
    for(int j=0; j< ev_ve.at(i).size(); j++){
      ...
      I.at(i).at(j)=2*pi*da_r*sin(ang_r1[j])
	/(sin(ang_r1[j]/2)*sin(ang_r1[j]/2)
	  *sin(ang_r1[j]/2)*sin(ang_r1[j]/2))
	+2*pi*da_r*sin(ang_r1[j])
	/(sin((pi-ang_r1[j])/2)*sin((pi-ang_r1[j])/2)
	  *sin((pi-ang_r1[j])/2)*sin((pi-ang_r1[j])/2));      
	...
	setbeam=false;
      ...
      if(n_beam1.at(i).at(j)<0){cout<<"end the loop at"<<l1[i]<<" mm~"<<l1[i]+dz<<" mm, "<<ang1[j]<<" deg~"<<ang1[j]+da<<" deg because beam particle is 0."<<endl;
	fin_beam1=true;
	break;
      }
      else{
    ...
    if(fin_beam1){break;}    
    //    if(z1_max<l1[i]+dz){
    if(ev_ve.at(i).size()<1){
      cout<<"                                                                 "<<endl;
      cout<<"end the loop at "<<l1[i]<<" mm~"<<l1[i]+dz<<" mm, because of no reaction in over "<<l1[i]+dz<<" mm"<<endl;
  ...
  for(int i=0; i< ev_ve.size(); i++){
	...
	dep1=l1[i];
	dir1=ang1[j];
	Y1_eve=Y1_da.at(i).at(j)/ev_ve.at(i).at(j).size();
    ...
    //need
    cout<<"Filling data "<<100*i/ev_ve.size() <<" % !  \r"<<flush;
  }
