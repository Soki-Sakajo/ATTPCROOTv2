...
int ruther_macro_c12_each_angle_0316_24(Int_t subnum=1){ 
  ...
  TH1F *h_rmax_nd=new TH1F("h_rmax_nd","rmax_nd:reaction_rate",nbin,0,300);
  ...
  rutdata->Branch("rmax_nd_c12",&rmax1_nd,"rmax1/D");
  ...
  for(int i=0; i< ev_ve1.size(); i++){
	...
	rmax1_nd=r_nd_ve.at(i).at(j).at(k);
	h_z_nY->Fill(l1[i]);
  ...
  cout<<"n_rmax1 "<<n_rmax1<<"    Y1sum "<<Y1_rmax<<"  n_rmax1_nd  "<<n_rmax1_nd<<"                   "<<endl;
