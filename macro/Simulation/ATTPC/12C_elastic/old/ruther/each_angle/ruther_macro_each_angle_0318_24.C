...
int ruther_macro_each_angle_0318_24(Int_t subnum=1){ 
  ...
  TF1 *f2=new TF1("f2","[0]*sin([1]*x)*(1/(sin([1]*x/2)*sin([1]*x/2)*sin([1]*x/2)*sin([1]*x/2)))",3,177);
  ...
  */
  ...
  h_thetaY1->Fit("f2","","",0,180);
  h_thetaY1->Draw("hist");
  Double_t chi2=f2->GetChisquare();
  Double_t ndf=f2->GetNDF();
  cout<<chi2/ndf<<endl;
  */
  ...
  */
  ...
  TString outcan = "data1/ruther_can_0318_" + to_string(subnum) + ".C";
  can->SaveAs(outcan);
  h_z->Write();
  h_rmax->Write();
  h_rmax_nd->Write();
