...
int ruther_macro_each_angle_0315_24(Int_t subnum=1){ 
  ...
  TChain *tree2=new TChain("anadata","anadata");
    for(int i=0; i<subnum; i++){
      TString refi="sim_macro_p_" + to_string(i) + ".root";
      TString direfi=dir+"/"+refi;
      tree2->Add(direfi);
      if(i==0){cout<<"read from  "<<refi<<flush;}
      if(i==subnum-1){cout<<"  to  "<<refi<<endl;}
  ...
  TH2F *h_ztheta_12c=new TH2F("h_ztheta_12c","reaction position:angle (12c12c)",400,0,400,100,0,100);
  ...
  TString outdata = "data1/rutdata_" + to_string(subnum) + ".root";
  ...
  */
  ...
  TString outcan = "data1/ruther_can_0315_" + to_string(subnum) + ".C";
  can->SaveAs(outcan);
  h_z->Write();
  h_rmax->Write();
  h_rmax_nd->Write();
