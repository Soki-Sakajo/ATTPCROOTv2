...
//int ruther_macro_c12_each_angle_0213_24(){
int ruther_macro_c12_each_angle_0213_24(Int_t subnum=0){ 
  ...
  */
  ...
  if(subnum>0){
    for(int i=0; i<subnum; i++){
      ...
      if(i==subnum-1){cout<<"read to "<<refi<<endl;}
    }
  }
  cout<<"end reading files"<<endl<<endl;
  
  //when can't use the loop, use blow
  //tree1->Add("data/sim_macro_c12_0.root");
  //tree1->Add("data/sim_macro_c12_1.root");
  //tree1->Add("data/sim_macro_c12_2.root");
  //tree1->Add("data/sim_macro_c12_3.root");
  ...
  for(int j=0; j<Entry1; j++){
    ...
    if(theta1-theta_min<0){continue;}
  ...
  for(int i=0;i < ev_ve.size(); i++){
	...
	if(i==i1_max){
	  if(ev_ve.at(i).at(j).size()>0){j1_max=j;}
	}
      }
    }
    if(n1[i]<1){continue;}
