...
int ruther_macro_test_nd_0315_24(Int_t subnum=1){ 
  ...
  TChain *tree2=new TChain("anadata","anadata");
    for(int i=0; i<subnum; i++){
      TString refi="sim_macro_p_nd_0317_" + to_string(i) + ".root";
      TString direfi=dir+"/"+refi;
      tree2->Add(direfi);
      if(i==0){cout<<"read from  "<<refi<<flush;}
      if(i==subnum-1){cout<<"  to  "<<refi<<endl;}
  ...
  TH2F *h_ztheta_12c=new TH2F("h_ztheta_12c","reaction position:angle (12c12c)",400,0,400,100,0,200);
  ...
  TH2F *h_ztheta_p=new TH2F("h_ztheta_p","reaction position:angle (12cp)",400,0,400,100,0,200);
  ...
  TH1F *h_thetaY2=new TH1F("h_thetaY2","angle :reaction_rate",90,0,180);
  TH1F *h_thetaY3=new TH1F("h_thetaY3","angle :reaction_rate",100,0,200);

  TF1 *f1=new TF1("f1","[0]*sin([1]*x)*(1/(sin([1]*x/2)*sin([1]*x/2)*sin([1]*x/2)*sin([1]*x/2))+1/(cos([1]*x/2)*cos([1]*x/2)*cos([1]*x/2)*cos([1]*x/2)))",3,177);
  TF1 *f2=new TF1("f2","[0]*sin([1]*x)*(1/(sin([1]*x/2)*sin([1]*x/2)*sin([1]*x/2)*sin([1]*x/2))+1/(cos([1]*x/2)*cos([1]*x/2)*cos([1]*x/2)*cos([1]*x/2)))",3,177);

  //move to other file
  //  TH1F *h_z_nY=new TH1F("h_z_nY","reaction position:generated_rate",nbin_zn,0,400);
  //  TH1F *h_rmax_nY=new TH1F("h_rmax_nY","rmax:generaetd_rate",nbin,0,300);
  ...
  TString outdata = "data1/rutdata_test_nd_" + to_string(subnum) + ".root";
  ...
  for(int j=0; j<Entry1; j++){
    ...
    //    if(n_p1==2 && fabs(z1[0]-z1[1])<dz && theta1-theta_min>0 && theta1<=90){      
    if(n_p1==2 && fabs(z1[0]-z1[1])<dz && theta1-theta_min>0){
      ...
      ang_an1[z_n1][a_n1].push_back(theta1_an);
	...
	//	  h_thetaY1->Fill(theta1_cms[1],1);
	h_thetaY1->Fill(theta1_cms[0],1);
      }
      */
  ...
  for(int j=0; j<Entry2; j++){
    ...
    /*
    //need to modify
    if(pdg2[0]==pdg12c){theta2=theta2_cms[0];}
    //    if(pdg2[0]==pdgp){theta2=theta2_cms[0];}
    else if(pdg2[1]==pdg12c){theta2=theta2_cms[1];}
    //    else if(pdg2[1]==pdgp){theta2=theta2_cms[1];}
    else if(pdg2[0]==pdg12c && pdg2[1]==0){
      //      cout<<j<<" this is beam event"<<endl;
    }
    else{cout<<j<<" something wrong with pdg"<<endl;}
    //to here
    */
  ...
  */
  ...
  /*
  for(int m=0; m<=nbin_zn;m++){
    double err_z=sqrt(h_z_nY->GetBinContent(m));
    h_z_nY->SetBinError(m,err_z);
  }
  for(int m=0; m<=nbin;m++){
    double err=sqrt(h_rmax_nY->GetBinContent(m));
    h_rmax_nY->SetBinError(m,err);
  }
  */
  ...
  */
  ...
  //output for canvas
  TString outcan = "data1/ruther_test_can_nd_0315_" + to_string(subnum) + ".C";
  can->SaveAs(outcan);
  h_z->Write();
  h_rmax->Write();
  h_rmax_nd->Write();
