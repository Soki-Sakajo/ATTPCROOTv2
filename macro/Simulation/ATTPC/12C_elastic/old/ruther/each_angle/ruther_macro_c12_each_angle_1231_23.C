...
int ruther_macro_c12_each_angle_1231(Int_t subnum=0){ 
  ...
  */
  ...
  TH2F *h_ztheta=new TH2F("h_ztheta","depth:angle",400,0,400,180,0,180);
  ...
  for(int i=0;i < div_l ; i++){
    ...
    for(int j=0; j< Entry1; j++){
      ...
      if(j%2==1 && fabs(z1[0]-z1[1])<dz && z1[0]>=l1[i] && z1[0]<l2[i]){
	h_ztheta->Fill(z1[0],theta1_cms[0]);
	h_ztheta->Fill(z1[1],theta1_cms[1]);
	...
	continue;
      ...
      //      I[j]=2*pi*dtheta_r*sin(theta1_cms_r)/(sin(theta1_cms_r/2)*sin(theta1_cms_r/2)*sin(theta1_cms_r/2)*sin(theta1_cms_r/2));
  ...
  TCancas *can=new TCanvas("hists","hists");
  can->Divide(2,2);
  ...
  TString outfname = "data1/ruther12c12c_" + to_string(subnum) + ".root";
