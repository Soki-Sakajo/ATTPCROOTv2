...
int ruther_macro_c12_all_angle(){ 
  ...
  //  for(int i=0;i <100; i++){
    ...
    if(n_beam[i]<0){
      div_ll=l1[i];
      cout<<"end the loop at"<<l1[i]<<" mm~"<<l2[i]<<" mm                   "<<endl; 
      break;
    }   
    if(n1[i]<1){
      continue;
    ...
    //      if(i>=350){cout<<i<<"     "<<n_beam[i]<<"         "<<sigma[i]<<"      "<<Y1[i]<<"         "<<Y1_sum<<"     "<<Eave1[i]<<endl}
  ...
  cout<<"n_rmax"<<n<<"    Y1sum"<<Y1_rmax<<"                               "<<endl;
  ...
  h_z->SetStats(0);
  //  h_z->Draw("hist");
  h_z->SaveAs("data/ruther_macro_result_c12_all_angle_depth.C");

  // rmax-reaction rate
  h_rmax->SetStats(0);
  h_rmax->Draw("hist");
  h_rmax->SaveAs("data/ruther_macro_result_c12_all_angle_rmax.C");
