...
int ruther_macro_each_angle_nd_0610_24(Int_t subnum=1){ 
  ...
  for(int i=0; i< ev_ve1.size(); i++){
	...
	if(rmax1>200){
	  cout<<"strange event?  "<<n_fi1<<"  "<<n_ev1<<"  "<<dep1<<"  "<<rmax1<<endl;
	}
	/*
  ...
  */
  ...
  //output for canvas
  TString outcan = "data1/ruther_can_nd_0610_" + to_string(subnum) + ".C";
  can->SaveAs(outcan);
  h_z->Write();
  h_rmax->Write();
  h_rmax_nd->Write();
