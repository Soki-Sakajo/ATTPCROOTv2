...
int comp_reaction_ruther_130matm(Int_t subnum=30){
  ...
  for(int i=0;i < ev_ve2.size(); i++){
	...
	cout<<"end the loop at"<<l2[i]<<" mm~"<<l2[i]+dz<<" mm, "
	    <<ang2[j]<<"deg~"<<ang2[j]+da<<"deg, because beam particle is 0."<<endl<<endl;
	cout<<"max point at "<<l2[i]<<" mm~"<<l2[i]+dz<<" mm "
  ...
  TCanvas *can1=new TCanvas("hists","hists",1500,1000);
  can1->cd(1);
  gPad->SetLogy(1);
  ...
  h_z->SetMinimum(1e-1);
  h_z->SetMaximum(2e4);
  h_z->SetTitle("Depth distribution (Rutherford scattering 12c12c and 12cp)");
  h_z->GetXaxis()->SetTitle("Depth in AT-TPC [mm]");
  h_z->GetYaxis()->SetTitle("reaction rate [cps]");
  h_z->Draw("hist");
  h_z->SetTitleSize(0.08);
  h_z->GetXaxis()->SetLabelSize(0.08);
  h_z->GetYaxis()->SetLabelSize(0.08);
  h_z->GetXaxis()->SetTitleSize(0.08);
  h_z->GetYaxis()->SetTitleSize(0.08);
  h_z->GetXaxis()->SetTitleOffset(0.9);
  h_z->GetYaxis()->SetTitleOffset(0.8);
  gStyle->SetOptTitle(0);
  gPad->Update();
  TLatex latex1;
  latex1.SetTextSize(0.08);
  latex1.SetTextColor(kBlack);
  latex1.DrawLatex(20,2.5e3,"#splitline{^{12}C beam @ 50 MeV}{iso-C_{4}H_{10} 0.13 atm.}");
  can1->Update();
  ...
  h_rmax->GetXaxis()->SetLabelSize(0.08);
  h_rmax->GetYaxis()->SetLabelSize(0.08);
  h_rmax->GetXaxis()->SetTitleSize(0.08);
  h_rmax->GetYaxis()->SetTitleSize(0.08);
  h_rmax->GetYaxis()->SetTitleOffset(1);
  gStyle->SetOptTitle(0);
  gPad->Update();
  TLatex latex2;
  latex2.SetTextSize(0.06);
  latex2.SetTextColor(kBlack);
  latex2.DrawLatex(150,3.5e3,"#splitline{^{12}C beam @ 50 MeV}{iso-C_{4}H_{10} 0.13 atm.}");
  /*
  ...
  */
  TString outcan1="./pdf_output/can_rut_130_dep.pdf";
  TString outcan2="./pdf_output/can_rut_130_rmax.pdf";
