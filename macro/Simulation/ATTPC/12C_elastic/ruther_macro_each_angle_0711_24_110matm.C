...
int ruther_macro_each_angle_0711_24_110matm(Int_t subnum=30){
  ...
  Int_t max_hist=500; // bin of z-reaction_rate hist
  ...
  for(int i=0; i<subnum; i++){
    ...
    TString refi="sim_macro_12c_110matm_" + to_string(i) + ".root";
  ...
  for(int i=0; i<subnum; i++){
    TString refi="sim_macro_p_110matm_" + to_string(i) + ".root";
    TString direfi=dir+"/"+refi;
    tree2->Add(direfi);
    if(i==0){cout<<"read from  "<<refi<<flush;}
    if(i==subnum-1){cout<<"  to  "<<refi<<endl;}
  ...
  TString outdata = dir+"/rutdata_0711_24_110matm_" + to_string(subnum) + ".root";
  ...
  */
  ...
  //output for canvas
  TString outcan = dir+"/ruther_can_0711_110matm_" + to_string(subnum) + ".C";
  can->SaveAs(outcan);
  h_z->Write();
  h_rmax->Write();
  h_rmax_nd->Write();
