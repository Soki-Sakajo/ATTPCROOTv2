...
int ruther_macro_each_angle_0711_24_80matm(Int_t subnum=30){
  ...
  Int_t n_b=1e4;        // /s; beam intensity
  ...
  for(int i=0; i<subnum; i++){
    ...
    TString refi="sim_macro_12c_80matm_" + to_string(i) + ".root";
  ...
  for(int i=0; i<subnum; i++){
    TString refi="sim_macro_p_80matm_" + to_string(i) + ".root";
    TString direfi=dir+"/"+refi;
    tree2->Add(direfi);
    if(i==0){cout<<"read from  "<<refi<<flush;}
    if(i==subnum-1){cout<<"  to  "<<refi<<endl;}
  ...
  TString outdata = dir+"/rutdata_0711_24_80matm_" + to_string(subnum) + ".root";
  ...
  */
  ...
  //output for canvas
  TString outcan = dir+"/ruther_can_0711_80matm_" + to_string(subnum) + ".C";
  can->SaveAs(outcan);
  h_z->Write();
  h_rmax->Write();
  h_rmax_nd->Write();
