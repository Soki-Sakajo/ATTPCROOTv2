...
int search_rmax(Int_t subnum=17){ 
  ...
  //  for(int i=0;i < 100; i++){
	...
	*/
	...
	for(int k=0; k<np ; k++){
	  ...
	  if(trackid==j){
	    ...
	    if(i==23871 && j==1 ){
	      cout<<i<<"  "<<j<<"  "<<k<<"  mass  "<<masstemp[j]<<"  atom  "<<atomtemp[j]<<"   x "<<stopx[j]
		  <<"   y "<<stopy[j]<<"   z "<<stopz[j]<<endl;
	    }
	...
	*/
	...
	//	if(j==1){
	if(i==1 && j==1){
	  if(fabs(pbx-(px[0]+px[1]))+fabs(pby-(py[0]+py[1]))+fabs(pbz-(pz[0]+pz[1]))>1){
	    ne_p1++;
	    ...
	    cout<<"event "<<i<<" n "<<ne_p1<<endl<<endl;
