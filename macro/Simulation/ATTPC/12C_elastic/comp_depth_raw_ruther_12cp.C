...
int comp_depth_raw_ruther_12cp(Int_t subnum=30){
  ...
  for(int i=0;i < ev_ve2.size(); i++){
    ...
    if(n2[i]>0){
      h_z_12c->Fill(l2[i],Y2[i]);
      h_en_12c->Fill(l2[i],Y2[i]);
    }
    if (int(l2[i])%100==0){
      //need
  ...
  h_gene_12c->SetStats(0);
  h_gene_12c->SetTitle("Energy distribution (uniform energy loss) (12cp)");
  h_gene_12c->GetXaxis()->SetTitle("Depth in AT-TPC [mm]");
  h_gene_12c->GetYaxis()->SetTitle("generated rate  N(z) #propto E_{beam}");
  h_gene_12c->Draw("hist");
  h_gene_12c->SetTitleSize(0.08);
  h_gene_12c->GetXaxis()->SetLabelSize(0.06);
  h_gene_12c->GetYaxis()->SetLabelSize(0.06);
  h_gene_12c->GetXaxis()->SetTitleSize(0.06);
  //  h_gene_12c->GetXaxis()->SetTitleOffset(0.8);
  h_gene_12c->GetYaxis()->SetTitleSize(0.06);
  h_gene_12c->GetYaxis()->SetTitleOffset(1.1);
  //  gStyle->SetOptTitle(0);
  gPad->Update();
  ...
  h_z_12c->SetTitle("Rutherford scattering energy distribution (12cp)");
  ...
  //  latex3.DrawLatex(80,1.5e3,"#propto E^{-2}_{z}");
  ...
  */
  TString outcan1="./pdf_output/can_12cp_depth_raw.pdf";
  TString outcan2="./pdf_output/can_12cp_depth_rut.pdf";
